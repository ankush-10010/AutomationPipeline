...
def run_tts(
    ...
    if not check_piper_installed():
        log.error("Piper TTS is not installed. Please run: pip install piper-tts")
        return None
    ...
    log.info("Generating local voice cloning audio using model: %s", model)
...
def run_assemble(
    ...
    # --- Pause for Colab Image Generation ---
    missing_images = []
    for seg in manifest.get("segments", []):
        if seg.get("visual_type") == "ai_image":
            img_file = images_dir_path / f"seg_{seg['id']:04d}.png"
            if not img_file.exists():
                missing_images.append((seg['id'], seg.get("visual_source")))
    
    if missing_images:
        print("\n" + "=" * 60)
        print("⚠️  MISSING AI IMAGES DETECTED")
        print("=" * 60)
        print("\nYour local script needs AI-generated images for scenes with no matching clips.")
        print("Since you want to generate images on Colab, please do the following:")
        print("1. Upload 'output/manifest.json' to your Colab notebook.")
        print("2. Run the Colab notebook to generate the images.")
        print("3. Download the generated images into your local 'images/' folder.")
        print("\nMissing Images:")
        for seg_id, prompt in missing_images:
            print(f"  - seg_{seg_id:04d}.png")
            print(f"    Prompt: {prompt}")
        print()
        input("Press Enter once the images are in your local 'images/' folder to continue assembling...")
